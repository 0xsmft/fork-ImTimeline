...
void ImTimeline::TimelinePlayer::Update( f32 aDeltaTime )
{
	...
	if( mPlayingNode == nullptr ) 
	{
		...
		mPlayingNodeProperties.mState = PlayingNodeState::None;
	...
	{
		if( mPlayingNodeProperties.mState == PlayingNodeState::None ) 
		{
			// play node
			...
			mPlayingNodeProperties.mState = PlayingNodeState::IsPlayed;
		}

		if( mPlayingNodeProperties.mState == PlayingNodeState::IsPlayed && mTimeStep.GetTimestamp() >= ( f32 ) mPlayingNode->End ) {
			//end play
			if( mPlayingNode->GetCustomNode() ) 
			{
			...
			mPlayingNodeProperties.mState = PlayingNodeState::IsFinishedPlaying;
			mPlayingNode = nullptr;
		}
	}
...
void ImTimeline::TimelinePlayer::Play()
{
	...
	{
		Setup( mTimelineData, ( s32 ) mTimeStep.GetTimestamp() );
	}
	...
	mTimeStep.SetTimestamp( ( s32 ) timestamp );
