...
namespace ImTimeline {
	...
	Timeline::Timeline()
	{
		m_EmptyDummyNode.Flags.set( eTimelineNodeFlags::TimelineNodeFlags_UseSectionBackground, true );
		m_EmptyDummyNode.Flags.set( eTimelineNodeFlags::TimelineNodeFlags_AutofitHeight, true );
		m_EmptyDummyNode.Flags.set( eTimelineNodeFlags::TimelineNodeFlags_MoveSurroundingNodesToTheRight, true );
		m_EmptyDummyNode.Flags.set( eTimelineNodeFlags::TimelineNodeFlags_MovedToDifferentTimeline, false );
	...
	TimelineNode* Timeline::AddNewNode( TimelineNode* node )
	{
		...
		if( m_Timelines[ node->m_Section ].pNodeData == nullptr ) {
			IM_ASSERT( false ); // arienai
		}

		// Node is not new, apply existing settings
		if( node->Flags.test( eTimelineNodeFlags::TimelineNodeFlags_MovedToDifferentTimeline ) ) {
			// good!
		}
		else {
			...
			if( node->m_CustomNode != nullptr ) {
				node->Flags.set( eTimelineNodeFlags::TimelineNodeFlags_CustomDraw, true );
			}
		...
		bool bMoveOverlap = node->Flags.test( eTimelineNodeFlags::TimelineNodeFlags_MoveSurroundingNodesToTheRight );
	...
	void Timeline::Undo()
	{
		if( m_CommandIndex >= 0 ) 
		{
			m_CommandHistory[ m_CommandIndex ]->command_undo();
			--m_CommandIndex;
		}
	}
	...
	void Timeline::Redo()
	{
		...
		{
			++m_CommandIndex;
			m_CommandHistory[ m_CommandIndex ]->command_do();
		}
	}
	...
	bool Timeline::DrawTimeline()
	{
		...
		for( auto& timeline : m_Timelines ) 
		{
			...
			const size_t localCustomHeight = timeline.second.mProps.DisplayProperties.Height;
			const ImVec2 pos = ImVec2( contentMin.x, contentMin.y + 1 + customHeight );
		...
		if( IsDragging() && m_pSelectedNode != nullptr ) 
		{
			...
			const f32 nodeWith = m_pSelectedNode->End - m_pSelectedNode->Start;
			m_DragData.DragNode.End = m_DragData.DragNode.Start + nodeWith;
	...
	void Timeline::DrawHeader( const ImRect& headerRect )
	{
		...
		{
			int window_x = ( int ) canvasPos.x + int( i * m_Zoom ) + m_Style.LegendWidth - int( m_StartFrame * m_Zoom );
			int window_y = canvasPos.y + height;
			pDrawList->AddLine( ImVec2( window_x, canvasPos.y ), ImVec2( window_x, window_y ), m_Style.HeaderTimeStampColor, 1 );

			if( bDrawLabel ) 
			{
				std::string label;
				ImTimelineUtility::sprint_f( label, "%d", i );
				pDrawList->AddText( ImVec2( ( float ) window_x + 3.f, canvasPos.y ), m_Style.HeaderTimeStampColor, label.c_str() );
			}
		};
	...
	void Timeline::OnCoreDebugGUI()
	{
		...
		if( ImGui::TreeNodeEx( "Navigation" ) ) 
		{
			if( m_MainPlayer ) 
			{
				s32 currentTimestamp = m_MainPlayer->GetCurrentTimestamp();
		...
		if( ImGui::TreeNodeEx( "Display Style" ) )
		{
			...
			ImGui::DragInt( "Header height", &m_Style.HeaderHeight, 1.f, 10.f, 300.f );
			ImGui::DragFloat( "Legend Width", &m_Style.LegendWidth, 1.f, 10.f, 300.f );
			ImGui::DragInt( "Scrollbar Thickness", &m_Style.ScrollbarThickness, 1.f, 10.f, 300.f );
	...
	s32 Timeline::GetTimestampAtPixelPosition( f32 pixelPos ) const
	{
		...
		return ( start / m_Zoom ) + m_StartFrame;
	}

	s32 Timeline::GetPixelPositionAtTimestamp( s32 timestamp ) const
	{
		return ( timestamp - m_StartFrame ) * m_Zoom;
	}
