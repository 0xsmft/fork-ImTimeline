...
void HorizontalNodeView::DrawNodeView( const ImRect& area, const TimelineSection& timeline, ImTimeline::Timeline* pContext )
{
	...
	size_t sectionHeight = timeline.mProps.DisplayProperties.Height;
	...
	rItemList->Iterate( [ & ]( TimelineNode& node ) 
	{
		...
		if( node.Flags.test( eTimelineNodeFlags::TimelineNodeFlags_AutofitHeight ) == false )
		{
		...
		const s32 scale = pContext->GetScale();
		...
		ImTimelineUtility::sprint_f( nodeDebugText, "id: %d - index: %d", node.m_ID, index );
...
void HorizontalNodeView::DefaultNodeDraw( const ImRect& area, const TimelineNode& node, ImTimeline::Timeline* timeline )
{
	...
	if( node.GetCustomNode() && node.Flags.test( eTimelineNodeFlags::TimelineNodeFlags_CustomDraw ) ) 
	{
	...
	if( node.Flags.test( eTimelineNodeFlags::TimelineNodeFlags_UseSectionBackground ) ) 
	{
	...
	pDrawList->AddText( area.Min + ImVec2( node.DisplayProperties.AccentThickness, node.DisplayProperties.AccentThickness ), node.DisplayProperties.ForegroundColor, node.DisplayText.c_str() );
...
{
	ImGui::Text( "Nodes Draw Skipped: %d", m_NodesDrawSkipped );
}
