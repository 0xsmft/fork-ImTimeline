...
namespace ImTimeline::Internal {
	...
	void MoveNodeCommand::CommandDo()
	{
	...
	void MoveNodeCommand::CommandUndo()
	{
		CommandDo();
	}

	void DeleteCommand::CommandDo()
	{
	...
	void DeleteCommand::CommandUndo()
	{
	...
	void ShowTimelineNodeFlagsDebugUI( TimelineNode* node )
	{
		...
		for( u32 i = 0; i < TimelineNodeFlags_Max; ++i )
		{
			...
			if( ImGui::Checkbox( ImTimelineNodeFlagsToString( flag ), &bSet ) )
			{
	...
	{
		auto container = new VectorContainer( TIMELINE_RESERVE_NODE_COUNT );
		return container;
	}
