...
void ImTimelineInternal::AddCommand::CommandDo()
{
...
void ImTimelineInternal::AddCommand::CommandUndo()
{
	IM_ASSERT( m_pTimeline != nullptr );
...
void ImTimelineInternal::MoveNodeCommand::CommandDo()
{
	...
	else 
	{
		...
		pNodeToMove->End = NewStart + ( s32 ) nodeWidth;
...
void ImTimelineInternal::MoveNodeCommand::CommandUndo()
{
	CommandDo();
}

void ImTimelineInternal::DeleteCommand::CommandDo()
{
...
void ImTimelineInternal::DeleteCommand::CommandUndo()
{
...
void ImTimelineInternal::ShowTimelineNodeFlagsDebugUI( TimelineNode* node )
{
	...
	for( u32 i = 0; i < TimelineNodeFlags_Max; ++i )
	{
		const auto flag = static_cast< TimelineNodeFlags >( i );
		bool bSet = node->Flags.test( flag );

		if( ImGui::Checkbox( TimelineNodeFlagsToString( flag ), &bSet ) )
		{
