...
namespace ImTimeline {
	...
	bool TimelinePlayer::IsRootTimeline() const
	{
		return m_TimelineData == nullptr && m_Players.size() > 0;
	}

	void TimelinePlayer::SetViewUI( std::shared_ptr<ITimelinePlayerView> newView )
	{
		m_PlayerView = newView;
	}

	void TimelinePlayer::Update( f32 aDeltaTime )
	{
	...
	void TimelinePlayer::DrawPlayer()
	{
		if( m_PlayerView )
		{
			m_PlayerView->Draw();
	...
	void TimelinePlayer::OnDebugGUI()
	{
	...
	void TimelinePlayer::ChangeState( TimelineState aState )
	{
	...
	TimelineNode* TimelinePlayer::GetNextNodeToPlay()
	{
